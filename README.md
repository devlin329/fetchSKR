...
## 5. 關於鏈上數據延遲 (Lazy Update)

區塊鏈上的某些協議為了節省運算資源 (Compute Units)，採用 **Lazy Update** 機制。這意味著：

- 當沒有人對質押池進行操作 (存款/提款) 時，全域狀態 (Global State) 的數據不會自動更新。
- 只有當某個用戶觸發合約互動時，合約才會一次性結算過去這段時間累積的所有獎勵。

**這造成的影響**:
- 您在鏈上讀到的 `Total Staked` 和 `Exchange Rate` 可能是好幾天前的舊數據。
- 直接計算出來的餘額會少於實際應得的餘額 (因為少了這幾天的複利)。

### 自動補償機制 (Compensation Logic)

為了讓查詢結果與官方網站 (可能透過其他後端即時計算) 一致，我們的腳本內建了補償邏輯：

1. **偵測最後更新時間**: 掃描鏈上數據，自動判斷數據停留在何時 (例如 Feb 2)。
2. **計算時間差**: `Time Diff = 現在時間 - 最後更新時間`。
3. **補償利息**: 
   > 補償額 = 當前餘額 × 年化報酬率 (20.9%) × (時間差 / 365天)

如果腳本無法自動偵測到準確的時間戳記 (例如數據格式被隱藏)，則會啟用 **Fallback Mode**，根據已知的歷史缺口 (約 4.6 天) 進行估算補償。
